...
definition: A margem entre a capacidade <span class="accent2">máxima</span> de um órgão e a capacidade que o repouso <span class="accent2">exige</span> dele.
points:
...
definition: Com o envelhecimento, as camadas íntima e média vasculares <span class="accent2">triplicam de espessura</span> entre os 20 e os 90 anos.
points:
...
definition: O sistema respiratório alcança sua função máxima entre os <span class="accent2">20 e 25 anos</span> e declina progressivamente a partir daí.
points:
...
```mermaid
flowchart LR
  Figado["↓ fluxo hepático<br/>↓ citocromo P-450 (fase I)"] --> Clear["↓ clearance do fármaco"]
  Rim["↓ filtração glomerular"] --> Clear
  Clear --> Meia["↑ meia-vida<br/>↑ concentração plasmática"]
  Meia --> Cog["Efeitos adversos<br/>sedação · confusão · lentificação"]
```
...
term: Polifarmácia
definition: Uso simultâneo de <span class="accent2">cinco ou mais</span> medicamentos — situação frequente em idosos com múltiplas comorbidades.
points:
...
definition: Conjunto de alterações imunes do idoso que determinam maior suscetibilidade a <span class="accent2">infecções, neoplasias e doenças autoimunes</span>.
points:
...
```mermaid
flowchart TD
  Comorb["Comorbidade sistêmica<br/>hipertensão · diabetes · DPOC · doença renal"] --> Mec
  Mec["Mecanismos intermediários<br/>hipoperfusão · hipóxia · inflamação · alteração metabólica"] --> Vuln
  Vuln["Vulnerabilidade neuronal<br/>menor reserva e menor capacidade de reparo"] --> Cog
  Cog["Repercussão cognitiva<br/>atenção · memória · velocidade de processamento"]
  Farm["Fármacos<br/>meia-vida prolongada"] --> Cog
  Sens["Perdas sensoriais"] --> Cog
```
...
definition: A perda de um motoneurônio leva junto as <span class="accent2">fibras musculares que ele inervava</span> — e, ao contrário do que ocorre no jovem, não há regeneração por brotamento nos terminais intactos.
points:
...
definition: O que o envelhecimento normal reduz de forma mais consistente <span class="accent2">não é o número de neurônios</span> — é a densidade de conexões entre eles.
points:
...
```mermaid
flowchart TD
  APP["APP<br/>proteína precursora do peptídeo β-amiloide"]
  APP -->|α-secretase| NaoAmi["Via não amiloidogênica<br/>não forma βA"]
  APP -->|β-secretase BACE| Beta["Clivagem β"]
  Beta -->|γ-secretase| BA["Peptídeos βA40 e βA42"]
  BA --> Acum["Acúmulo no terminal sináptico<br/>superprodução ou falha da neprilisina"]
  Acum --> Placa["Complexos fibrilares<br/><b>placas senis</b>"]
```
...
definition: Agregados <span class="accent2">intraneuronais</span> de proteína Tau hiperfosforilada e insolúvel, organizados em filamentos helicoidais pareados.
points:
...
```mermaid
flowchart TD
  Decl["Declínio estrutural e funcional<br/>volume · sinapses · substância branca · neurotransmissores"] --> Desafio
  Desafio["Desafio ao desempenho"] --> Scaf["Recrutamento de circuitos alternativos<br/><b>andaimes</b> — recrutamento bilateral e frontal"]
  Enriq["Fatores de enriquecimento<br/>escolaridade · atividade física · engajamento"] --> Scaf
  Scaf --> Desemp["Desempenho cognitivo<br/>mantido em muitos domínios"]
```
