# Banco de questões · Aula 01 — Conceitos e teorias sobre o envelhecimento

**Disciplina** Neuropsicologia II · FASM
**Base** deck `aulas/aula-01-conceitos-e-teorias-sobre-o-envelhecimento.md`; Neri, A. L. (2013), cap. 1, em Malloy-Diniz, Fuentes & Cosenza (orgs.), *Neuropsicologia do envelhecimento*.
**Total** 18 questões · 5 múltipla escolha · 3 verdadeiro ou falso · 3 afirmações corretas · 4 estudos de caso · 3 estilo ENADE/ENEM

> **Como usar.** Cada questão traz o tipo, o nível de dificuldade e o objetivo de aprendizagem. O gabarito e o comentário estão dentro de um bloco recolhível (`<details>`), que aparece fechado no GitHub e pode ser apagado na hora de montar a prova.
>
> **Critério das questões.** Nenhuma pergunta cobra ano de publicação, nome de amostra ou número decorado. Onde um autor aparece, ele vem no enunciado como referência, não como resposta.

| # | Tipo | Nível | Tema |
|---|---|---|---|
| 1 | Múltipla escolha | muito fácil | Senescência e envelhecimento biológico |
| 2 | Verdadeiro ou falso | muito fácil | "Idoso" e "velhice" como conceitos |
| 3 | Afirmações corretas | fácil | Os três paradigmas |
| 4 | Múltipla escolha | fácil | Defasagem estrutural e modernização |
| 5 | ENADE/ENEM | médio | Coorte × efeito de idade |
| 6 | Estudo de caso | médio | Dependência comportamental (M. Baltes) |
| 7 | Verdadeiro ou falso | médio | Atividade × afastamento |
| 8 | Múltipla escolha | médio | Erikson — integridade × desespero |
| 9 | Estudo de caso | médio | Transições normativas × idiossincráticas |
| 10 | Afirmações corretas | médio | Seletividade socioemocional |
| 11 | ENADE/ENEM | difícil | Leitura de dados de rede social |
| 12 | Estudo de caso | difícil | SOC — seleção, otimização e compensação |
| 13 | Múltipla escolha | difícil | Controle primário e secundário |
| 14 | Afirmações corretas | difícil | Dinâmica biologia–cultura e reabilitação |
| 15 | Verdadeiro ou falso | difícil | Rótulos da "boa velhice" e responsabilização |
| 16 | Múltipla escolha | muito difícil | Paradoxo do bem-estar |
| 17 | Estudo de caso | muito difícil | Integração — eventos críticos e agência |
| 18 | ENADE/ENEM | muito difícil | Teoria como decisão metodológica |

---

## Questão 1 · múltipla escolha · muito fácil
**Objetivo** distinguir envelhecimento biológico de velhice.

O envelhecimento biológico, também chamado de senescência, é definido como a diminuição progressiva da capacidade de adaptação e de sobrevivência. Sobre esse processo, é correto afirmar que ele:

**a)** é universal e determinado geneticamente na espécie, iniciando-se logo após a maturidade sexual e acelerando-se a partir da quinta década.
**b)** começa a partir dos 60 anos, idade em que a legislação brasileira reconhece a pessoa como idosa.
**c)** só ocorre em pessoas que acumularam doenças crônicas ao longo da vida.
**d)** é um processo exclusivamente anatômico, sem repercussão funcional.
**e)** é sinônimo de velhice, já que ambos descrevem a mesma fase do ciclo vital.

<details>
<summary>Gabarito e comentário</summary>

**Gabarito: a**

A definição é funcional, não anatômica — e é justamente isso que a torna útil à neuropsicologia: o que diminui é a *capacidade de adaptação*.

- **b)** confunde o processo biológico com o corte etário administrativo. Os 60 anos vêm da demografia e da previdência, não da fisiologia.
- **c)** contraria a universalidade do processo: senescência acontece com todos, com ou sem doença.
- **d)** inverte a definição, que é de capacidade e não de estrutura.
- **e)** é o erro central que a aula combate. Velhice é a *última fase do ciclo vital*, produto da ação concorrente de dois processos — desenvolvimento e envelhecimento. Reduzi-la ao segundo é apagar o primeiro.
</details>

---

## Questão 2 · verdadeiro ou falso · muito fácil
**Objetivo** separar o que é categoria sociocultural do que é processo biológico.

Julgue cada afirmação como **verdadeira (V)** ou **falsa (F)**.

| | Afirmação | V/F |
|---|---|---|
| I | "Idoso" é uma categoria sociocultural, definida por aparência, força, funcionalidade, produtividade e desempenho de papéis sociais. | |
| II | O corte etário que define quem é idoso é o mesmo em todos os países, porque decorre de um marco biológico. | |
| III | A velhice resulta da ação concorrente de dois processos simultâneos: desenvolvimento e envelhecimento. | |
| IV | Definir uma idade a partir da qual alguém é considerado idoso é também uma decisão política, porque atribui direitos e deveres. | |

<details>
<summary>Gabarito e comentário</summary>

**I — V.** É a definição do capítulo: uma categoria construída socialmente a partir de diferenças observáveis.

**II — F.** No Brasil o corte é 60; em boa parte da Europa, 65. A variação denuncia a origem previdenciária e demográfica do critério, não neurológica.

**III — V.** Os dois correm juntos a vida inteira; o que muda com a idade é a proporção entre eles, não a existência de um deles.

**IV — V.** É exatamente por isso que o Estatuto do Idoso precisa de um número: sem corte etário não há a quem garantir gratuidade, prioridade e proteção.
</details>

---

## Questão 3 · afirmações corretas · fácil
**Objetivo** reconhecer o que cada paradigma permite enxergar.

Considere as afirmações abaixo sobre os três paradigmas da psicologia do envelhecimento.

**I.** No paradigma de **ciclo de vida**, herdado da biologia, o ápice do desenvolvimento é a capacidade de reproduzir a espécie — e tudo o que vem depois é, por definição, contração.
**II.** O paradigma de **curso de vida**, de origem sociológica, descreve trajetórias construídas socialmente, atento a coortes, papéis e eventos.
**III.** O paradigma do **desenvolvimento ao longo da vida** (*lifespan*) sustenta que ganhos e perdas coexistem em todas as idades, e que o que muda com a idade é a alocação de recursos.
**IV.** Os três paradigmas descrevem o mesmo fenômeno com palavras diferentes e produzem, na prática, as mesmas hipóteses de pesquisa.

Estão corretas apenas:

**a)** I e II
**b)** II, III e IV
**c)** I e IV
**d)** I, II e III
**e)** III e IV

<details>
<summary>Gabarito e comentário</summary>

**Gabarito: d**

**IV é falsa** e é o ponto da questão: a moldura decide o que a pesquisa consegue enxergar. Quem parte de crescimento–culminância–contração não formula a pergunta "o que ainda se desenvolve aqui?" — a resposta já estava embutida na premissa. Foi por isso que a observação dos anos 1960 (idosos que *continuavam a se desenvolver* em domínios selecionados) quebrou o paradigma, e não apenas acrescentou um dado a ele.
</details>

---

## Questão 4 · múltipla escolha · fácil
**Objetivo** diferenciar duas explicações sociológicas para a perda de status do idoso.

Um município discute o fechamento do único centro de convivência para idosos do bairro, alegando que "o dinheiro rende mais na creche". Nas redes sociais, circula a ideia de que "idoso demais é o que quebra a saúde e a previdência".

A leitura desse cenário à luz da **teoria da defasagem estrutural** é a de que:

**a)** os idosos se afastam voluntariamente da vida social, e o fechamento apenas acompanha uma tendência natural.
**b)** a decisão é neutra do ponto de vista teórico, já que orçamento é matéria de gestão e não de psicologia do envelhecimento.
**c)** as estruturas sociais não conseguem oferecer aos idosos economicamente improdutivos os mesmos benefícios disponíveis aos membros produtivos — e o estereótipo do idoso como custo é subproduto dessa defasagem.
**d)** trata-se de um fenômeno individual, explicado pela baixa autoeficácia dos idosos do bairro.
**e)** o conhecimento acumulado pelos idosos ficou obsoleto diante das novas tecnologias, o que explica a decisão.

<details>
<summary>Gabarito e comentário</summary>

**Gabarito: c**

A defasagem estrutural diz que a sociedade não acompanha a própria mudança demográfica: continua organizada em torno da produtividade e não redistribui benefícios a quem saiu dela.

- **a)** é a teoria do afastamento, que não se sustenta empiricamente.
- **b)** ignora que a decisão orçamentária opera sobre um estereótipo com história documentada.
- **d)** individualiza um fenômeno estrutural — exatamente o movimento que a aula critica.
- **e)** descreve o **primeiro processo da teoria da modernização** (novas tecnologias tornando obsoleto o conhecimento acumulado) — é uma teoria vizinha, não esta. Vale conhecer as duas e não trocá-las.
</details>

---

## Questão 5 · ENADE/ENEM · médio
**Objetivo** distinguir efeito de idade de efeito de coorte.

> Em 1921, os resultados da testagem de cerca de 1,8 milhão de recrutas norte-americanos mostraram que os mais velhos tinham desempenho inferior aos mais jovens em testes de inteligência. A conclusão da época foi a de declínio biológico típico do envelhecimento. Só décadas depois se notou que os grupos comparados diferiam muito em escolaridade e em familiaridade com o formato dos testes.

Um serviço de saúde replica hoje o mesmo desenho: aplica, num único dia, a mesma bateria a um grupo de 30 pessoas com 25 anos e a um grupo de 30 pessoas com 75 anos, e conclui, da diferença encontrada, que houve declínio cognitivo com a idade.

Sobre esse raciocínio, é correto afirmar que:

**a)** é válido, desde que os dois grupos tenham sido testados pelo mesmo examinador e no mesmo ambiente.
**b)** é robusto, porque o tamanho amostral compensa as diferenças de escolaridade entre os grupos.
**c)** é inválido porque a idade cronológica não guarda nenhuma relação com o desempenho cognitivo.
**d)** é inválido apenas se os grupos diferirem em sexo, já que idade e escolaridade são variáveis independentes.
**e)** é frágil, porque diferenças entre grupos etários em estudos transversais devem ser tratadas como efeito de coorte até prova em contrário — os grupos não compartilham a mesma história de escolarização e de exposição cultural.

<details>
<summary>Gabarito e comentário</summary>

**Gabarito: e**

Coorte é o grupo que, por ter nascido no mesmo período histórico, compartilha experiências sociais — inclusive a qualidade e a duração da escola que teve. Num desenho transversal, efeito de idade e efeito de coorte estão somados e não podem ser separados.

- **b)** amostra grande não corrige viés sistemático: só torna o viés mais preciso.
- **c)** exagera na direção oposta. A idade tem efeito, mas ela não é *causa* — é índice de tempo vivido, e concorre com contexto e história como explicação.

**Ponte com a prática:** é o mesmo problema, em escala clínica, de aplicar a um idoso de 80 anos com 3 anos de escolaridade uma norma construída em outra população. Normas por escolaridade existem por causa deste erro.
</details>

---

## Questão 6 · estudo de caso · médio
**Objetivo** ler a dependência como padrão mantido por contingências, e não como traço.

> **Sr. Osvaldo, 81 anos, institucionalizado há dois anos.**
> **História de vida.** Ferroviário aposentado, viúvo, dois filhos que moram em outro estado. Até os 78 anos morava sozinho, cozinhava, cuidava de um pequeno quintal e ia à padaria todos os dias. Foi para a instituição depois de uma fratura de fêmur com boa recuperação funcional.
> **Queixa da equipe.** "Ele regrediu muito. Hoje não faz mais nada sozinho, quer tudo na mão."
> **Observação clínica em três dias de visita.** Quando o Sr. Osvaldo pede ajuda para se vestir, a cuidadora atende em menos de trinta segundos, senta na cama e conversa com ele durante o procedimento. Quando ele começa a se vestir sozinho, ninguém entra no quarto; em duas ocasiões pediu para tentar sozinho e ouviu "deixa que é mais rápido assim". Ele conclui a tarefa sozinho quando tem tempo, com lentidão e sem risco de queda.
> **Avaliação.** Desempenho cognitivo dentro do esperado para idade e escolaridade; sem indicativo de quadro depressivo; força e marcha preservadas com bengala.

Qual leitura integra melhor os dados?

**a)** O comportamento dependente é um traço de personalidade que emergiu com o envelhecimento e com a perda do papel de trabalhador.
**b)** Trata-se de perda de reserva funcional pela fratura, e a conduta é fisioterapia intensiva.
**c)** O comportamento dependente é um padrão aprendido, mantido por contingências do ambiente: a dependência é reforçada com atenção e contato social imediatos, enquanto as tentativas de independência entram em extinção.
**d)** O quadro indica declínio funcional progressivo e sugere transtorno neurocognitivo em fase inicial.
**e)** A institucionalização produziu depressão, e a dependência é sintoma dela.

<details>
<summary>Gabarito e comentário</summary>

**Gabarito: c**

É o script dependência–apoio descrito por M. M. Baltes: ajuda física e atenção social seguem a dependência; a tentativa de fazer sozinho é ignorada. O padrão prospera porque *funciona* — pedir ajuda é, naquele ambiente, a via mais confiável de obter contato social.

O que sustenta a alternativa contra as demais são três dados do caso: cognição preservada, ausência de indicativo depressivo e capacidade motora demonstrada quando há tempo. Não há déficit a explicar o comportamento — há uma contingência.

**A pergunta clínica correta** não é "o Sr. Osvaldo é dependente?", e sim "o quanto esse padrão é funcional e adaptativo *neste* ambiente, e o que o ambiente está reforçando?". A intervenção, portanto, não é no idoso: é no treino do cuidador para reforçar *tentativas*, e não resultados.

**Nuance que a turma costuma perder:** dependência não é necessariamente ruim. Ela pode ser funcional — poupa recursos para domínios mais importantes e obtém apoio em áreas genuinamente prejudicadas. O problema aqui é que ela substituiu um repertório que existe.
</details>

---

## Questão 7 · verdadeiro ou falso · médio
**Objetivo** avaliar criticamente o debate atividade × afastamento.

Julgue cada afirmação como **verdadeira (V)** ou **falsa (F)**.

| | Afirmação | V/F |
|---|---|---|
| I | A teoria da atividade sustenta que a velhice exitosa se caracteriza por alta satisfação, saúde e produtividade, e que cabe ao idoso substituir os papéis sociais perdidos. | |
| II | A teoria do afastamento descreve o desengajamento como natural, normal e mutuamente consentido entre o idoso e a sociedade. | |
| III | A teoria do afastamento foi confirmada por múltiplos estudos longitudinais, e hoje é o modelo dominante na psicologia do envelhecimento. | |
| IV | Se o afastamento for entendido como natural, ele deixa de ser objeto de política pública; se for entendido como estrutural, passa a sê-lo. | |

<details>
<summary>Gabarito e comentário</summary>

**I — V.** E ancorada no valor de que "mente sã é fruto de corpo são".

**II — V.** É a formulação da teoria: o idoso se prepara para sair, os jovens ocupam o espaço, e isso seria um requisito funcional da estabilidade social.

**III — F.** É o contrário. Não há evidência de afastamento voluntário e universal, e a teoria se apoiou essencialmente em um único estudo. A explicação hoje mais satisfatória é a **defasagem estrutural**: o idoso não se retira — ele é retirado, de forma gradual e diferencial.

**IV — V.** Esta é a consequência política do veredito, e vale explicitá-la: teorias descritivas produzem — ou impedem — condutas.

**Detalhe fino:** atividade e afastamento são teorias opostas ancoradas no *mesmo* valor cultural tradicional, e foram estabelecidas em complementaridade. Ambas influenciaram fortemente universidades da terceira idade e programas de preparação para a aposentadoria.
</details>

---

## Questão 8 · múltipla escolha · médio
**Objetivo** operacionalizar a crise da velhice em Erikson.

Na teoria do desenvolvimento psicossocial, a crise característica da velhice opõe **integridade do ego** e **desespero**, e a qualidade do ego que pode nascer dela é a sabedoria. Numa entrevista clínica, o achado que melhor corresponde ao polo do **desespero** descrito por essa teoria é:

**a)** a percepção de que não há mais tempo para refazer o que ficou por fazer, acompanhada de amargura ao revisar as próprias escolhas.
**b)** a queixa de que a memória piorou desde a aposentadoria.
**c)** humor deprimido persistente, insônia terminal e perda de peso nos últimos três meses.
**d)** desorientação temporal e dificuldade para reconhecer familiares.
**e)** a recusa em participar de atividades no centro de convivência do bairro.

<details>
<summary>Gabarito e comentário</summary>

**Gabarito: a**

O desespero de Erikson é uma posição existencial diante do tempo, não um quadro psiquiátrico. Ele **convive** com a depressão e às vezes a antecede, mas não é sinônimo dela — e a diferença muda a conduta: a intervenção não é reverter o tempo, é ressignificar o percurso. A técnica de revisão de vida nasce daqui.

- **c)** descreve um episódio depressivo, que exige conduta própria — e cuja confusão com o desespero eriksoniano é justamente o erro que a questão testa.
- **d)** é achado neurológico.
- **e)** e **b)** são dados isolados, compatíveis com muitas hipóteses.

**Consequência metodológica que Erikson deixa:** as tarefas evolutivas das crianças são universais; as dos idosos dependem muito mais da experiência pessoal. Quanto mais velho o sujeito, menos previsível pela idade e mais explicável pela biografia.
</details>

---

## Questão 9 · estudo de caso · médio
**Objetivo** aplicar a distinção entre transições normativas e idiossincráticas.

> **Duas viúvas, o mesmo evento.**
> **Dona Marlene, 79 anos.** Perdeu o marido há seis meses, após longa doença. Tem quatro amigas viúvas no grupo da igreja; três delas passaram pelo mesmo nos últimos cinco anos. Recebeu ajuda prática imediata (inventário, pensão, mudança de rotina) e diz que "todo mundo ali já sabe como é". Tristeza presente, funcionalidade preservada, retomou o coral no quarto mês.
> **Dona Priscila, 38 anos.** Perdeu o marido há seis meses, em acidente de trânsito. É a única pessoa viúva do seu círculo. Relata que as amigas "não sabem o que dizer" e mudam de assunto; foi orientada por colegas a "seguir em frente" já no segundo mês. Tem dois filhos pequenos, voltou ao trabalho no terceiro mês e descreve sensação constante de estar fora de lugar.

À luz da teoria do relógio social, qual leitura é a mais adequada?

**a)** O impacto tende a ser maior em Dona Marlene, pela proximidade da própria morte e pelo tempo de convivência perdido.
**b)** Como o evento é o mesmo, a teoria prevê impacto equivalente; a diferença é apenas de personalidade.
**c)** O luto de Dona Priscila é patológico e o de Dona Marlene é normal, porque o tempo de recuperação difere.
**d)** O impacto emocional tende a ser maior em Dona Priscila porque a transição é idiossincrática — rara, imprevisível, fora de qualquer prescrição cultural e vivida de forma solitária, sem repertório social prévio nem socialização antecipatória.
**e)** A diferença se explica integralmente pela presença de filhos pequenos, variável que a teoria não considera.

<details>
<summary>Gabarito e comentário</summary>

**Gabarito: d**

Transições **normativas** têm época esperada de ocorrência, são reconhecidas pela cultura, vividas junto com o grupo de idade e permitem socialização antecipatória — o que amortece o impacto. Transições **idiossincráticas** não têm nada disso.

O ponto que a questão quer fixar: **não é que o luto seja outro**. É que um tem roteiro social e o outro não. Dona Priscila enfrenta a perda *e* a ausência de repertório coletivo para lidar com ela — inclusive a pressa alheia ("siga em frente"), que é a tentativa do grupo de devolvê-la ao relógio.

- **c)** transforma diferença de contexto em diagnóstico, que é precisamente o erro.
- **e)** os filhos pequenos importam, mas eles são parte do que torna a transição fora do tempo esperado, não uma variável alheia à teoria.

**Pergunta clínica que a teoria deixa:** *de quem é o relógio que este paciente está usando?*
</details>

---

## Questão 10 · afirmações corretas · médio
**Objetivo** ler a redução da rede social como seleção ativa.

Sobre a teoria da **seletividade socioemocional**, considere:

**I.** A redução da rede social na velhice reflete perdas naturais e inevitáveis, e por isso deve ser lida como indicador de declínio.
**II.** O gatilho da mudança é a percepção de encurtamento da perspectiva de tempo futuro, e não a idade cronológica em si.
**III.** Metas de busca de informação cedem lugar a metas de regulação emocional, e a redução de contatos é uma seleção ativa.
**IV.** Jovens em estado terminal, quando perguntados sobre com quem gostariam de se relacionar, respondem de forma semelhante à dos idosos — o que sustenta que a variável em jogo é a perspectiva temporal.

Estão corretas apenas:

**a)** II e III
**b)** II, III e IV
**c)** I, II e III
**d)** I e IV
**e)** I, III e IV

<details>
<summary>Gabarito e comentário</summary>

**Gabarito: b**

**I é falsa** — e é exatamente a leitura que a teoria veio corrigir nas teorias clássicas do afastamento. A rede não encolhe uniformemente: **relações periféricas declinam** e **parceiros emocionalmente próximos se mantêm**. Isso não é decadência, é redistribuição de recursos socioemocionais.

O experimento de IV é o argumento decisivo, porque dissocia idade de horizonte temporal.

**Ligação com o metamodelo:** selecionar as relações significativas é *seleção*; investir nelas é *otimização*. A seletividade socioemocional é o modelo SOC no domínio socioemocional.
</details>

---

## Questão 11 · ENADE/ENEM · difícil
**Objetivo** interpretar dados e evitar a conclusão deficitária automática.

> Um serviço de atenção ao idoso acompanhou, em estudo longitudinal, dois indicadores de rede social em quatro momentos da vida adulta. Os valores estão em índice relativo (100 = número de contatos periféricos aos 20 anos).

| Idade | Contatos periféricos | Relações emocionalmente próximas |
|---|---|---|
| 20 anos | 100 | 30 |
| 40 anos | 82 | 32 |
| 60 anos | 55 | 31 |
| 80 anos | 28 | 29 |

Um estagiário conclui, a partir da tabela, que "a rede social se deteriora progressivamente com a idade, e o isolamento é um desfecho esperado da velhice". A crítica mais consistente a essa conclusão é:

**a)** a de que o índice relativo impede qualquer interpretação, já que não há valores absolutos.
**b)** a de que estudos longitudinais não permitem conclusões sobre mudança individual.
**c)** a de que a queda observada decorre integralmente de efeito de coorte.
**d)** a de que a tabela é insuficiente porque não informa o sexo dos participantes.
**e)** a de que a conclusão generaliza a queda de uma das linhas para as duas, quando o dado mostra declínio dos contatos periféricos e **estabilidade** das relações próximas — padrão compatível com seleção ativa, e não com deterioração.

<details>
<summary>Gabarito e comentário</summary>

**Gabarito: e**

A tabela contém duas trajetórias com formas distintas, e a conclusão do estagiário lê só uma delas. Ler a média das duas linhas, ou a linha maior, é o erro estatístico e o erro teórico ao mesmo tempo.

- **b)** inverte a virtude do desenho longitudinal, que é justamente medir mudança intraindividual.
- **c)** aplica a crítica de coorte fora de lugar: ela é o risco típico dos desenhos **transversais**.

**O que a leitura correta muda na conduta:** um idoso com menos contatos não é, por isso, um idoso em risco. O que se pergunta é se as relações **próximas** foram preservadas — e é a perda dessas, não das periféricas, que sinaliza problema.
</details>

---

## Questão 12 · estudo de caso · difícil
**Objetivo** identificar seleção, otimização e compensação num relato clínico.

> **Sr. Anselmo, 84 anos, professor de violão aposentado.**
> **História de vida.** Deu aulas por 45 anos, tocava em roda de choro três vezes por semana e mantinha repertório de cerca de sessenta peças de memória.
> **Situação atual.** Artrose nas mãos, dor ao final do dia e redução da velocidade de digitação. Refere que "não é mais o mesmo".
> **Observação em entrevista.** Reduziu o repertório para oito peças, que agora ensaia diariamente e com mais tempo do que ensaiava antes. Passou a escolher andamentos mais lentos nas passagens rápidas, e diz que "assim o contraste até fica melhor". Tocou em duas rodas no último mês e continua dando duas aulas por semana, agora só para alunos avançados. Relata boa satisfação com a vida e nenhuma queixa de humor.

A leitura correta do caso, à luz do metamodelo de seleção, otimização e compensação (SOC), é:

**a)** reduzir o repertório é **seleção**; ensaiar mais cada peça é **otimização**; ajustar o andamento para produzir contraste é **compensação** — e a satisfação preservada apesar da perda documentada é o desfecho que o modelo prevê.
**b)** o relato de satisfação apesar da perda sugere ausência de crítica sobre a própria condição, e deve ser investigado como anosognosia.
**c)** trata-se de dependência comportamental, mantida pela atenção dos alunos avançados.
**d)** a redução do repertório indica afastamento e prediz declínio; o caso ilustra a teoria do afastamento.
**e)** há um quadro de perda funcional com restrição de atividades, e a conduta é ampliar o repertório para evitar desengajamento.

<details>
<summary>Gabarito e comentário</summary>

**Gabarito: a**

O caso reproduz o exemplo canônico do próprio Baltes com o pianista Rubinstein, deslocado para um contexto brasileiro.

- **Seleção** — especificar e reduzir alternativas, reorganizando a hierarquia e o número de metas, quando tempo, energia e capacidade são limitados.
- **Otimização** — aplicação e manutenção de recursos para buscar níveis mais altos de funcionamento no que restou selecionado.
- **Compensação** — adoção de meios alternativos para manter o funcionamento.

**A alternativa **b** é a mais instrutiva das erradas.** Perder capacidade e continuar bem não é negação: é **mediação**. Um paciente com perda funcional documentada que relata boa qualidade de vida pode estar compensando muito bem — e chamar isso de falta de crítica é um erro clínico com consequências, porque autoriza intervenções que ele não precisa.

**A alternativa **e**** também merece nota: ampliar o repertório contraria a seleção que está sustentando o funcionamento. A conduta correta é apoiar a estratégia que já funciona.
</details>

---

## Questão 13 · múltipla escolha · difícil
**Objetivo** aplicar a régua de funcionalidade do controle secundário.

Uma paciente de 77 anos, após um período de dores e insônia, passou a usar um amuleto que ganhou de uma vizinha. Relata que "desde então melhorou". Objetivamente, a dor permanece a mesma; mas a ansiedade diminuiu, ela voltou a caminhar de manhã e retomou as visitas à filha.

A leitura correta desse quadro, segundo o modelo de controle primário e secundário, é:

**a)** trata-se de estratégia disfuncional, porque toda crença inválida enfraquece o controle a longo prazo.
**b)** trata-se de controle primário, já que a paciente alterou o ambiente ao adquirir o amuleto.
**c)** trata-se de ilusão **funcional**: a crença é inválida, mas a ação resultante é eficaz — e o critério que importa não é a veracidade da atribuição, e sim se ela aumenta o potencial de controle primário.
**d)** trata-se de sintoma psicótico incipiente, e a conduta é encaminhamento psiquiátrico.
**e)** trata-se de crença inválida, e a conduta é corrigir a atribuição causal, já que intervenções em saúde devem primar pela veracidade.

<details>
<summary>Gabarito e comentário</summary>

**Gabarito: c**

O quadro de referência cruza duas dimensões — verídico/ilusório e funcional/disfuncional — e o que decide não é a primeira, é a segunda.

|  | Funcional | Disfuncional |
|---|---|---|
| **Verídico** | ação eficaz a curto e longo prazo | eficaz agora, enfraquece o controle depois |
| **Ilusório** | ação eficaz, ainda que baseada em crença inválida | ação ineficaz baseada em crença inválida |

Aqui: a atribuição é inválida, a ansiedade caiu, a paciente voltou a se engajar. Crença inválida, efeito funcional.

**A régua clínica**, que vale copiar: reenquadramento que **devolve ação** é funcional; reenquadramento que **anestesia e paralisa** é disfuncional. Se a mesma paciente tivesse abandonado o tratamento da dor confiando só no amuleto, a mesma crença viraria ilusão disfuncional — o quadrante muda com a consequência, não com o conteúdo.

**Por que a alternativa **e** é tentadora e errada:** sob ameaça, ações que primam pela veracidade podem ser disfuncionais — criam desespero e desencorajam tentativas úteis de restabelecer o controle primário.
</details>

---

## Questão 14 · afirmações corretas · difícil
**Objetivo** derivar consequências de reabilitação dos princípios da dinâmica biologia–cultura.

Sobre os três princípios gerais da dinâmica entre biologia e cultura ao longo da vida, considere:

**I.** A plasticidade biológica e a fidelidade genética declinam com a idade, porque a seleção natural privilegiou as fases pré-reprodutiva e reprodutiva.
**II.** Estender o desenvolvimento até idades avançadas exige progressos culturais — higiene, imunização, habitação, educação.
**III.** Os mais velhos são **mais** responsivos aos recursos culturais do que os adultos jovens, o que explica o alto rendimento das intervenções em idades muito avançadas.
**IV.** Que a mesma intervenção renda menos aos 85 do que aos 65 anos é previsto pelo modelo — e não é argumento contra intervir.

Estão corretas apenas:

**a)** II, III e IV
**b)** I, II e IV
**c)** II e III
**d)** I e III
**e)** I, III e IV

<details>
<summary>Gabarito e comentário</summary>

**Gabarito: b**

**III inverte o terceiro princípio.** Os mais velhos são **menos** responsivos aos recursos culturais, porque a plasticidade e a resiliência biológicas são menores. É esse princípio que sustenta IV — e IV é a leitura clínica que interessa: rendimento decrescente não é rendimento nulo.

Este é o eixo teórico da reabilitação neuropsicológica no envelhecimento. O modelo prevê ao mesmo tempo (a) por que a cultura é indispensável para que exista velhice longa e funcional, e (b) por que ela tem limite. Quem só lê o segundo princípio vira otimista ingênuo; quem só lê o terceiro vira niilista terapêutico.
</details>

---

## Questão 15 · verdadeiro ou falso · difícil
**Objetivo** exercer a crítica ideológica dos rótulos da boa velhice.

Julgue cada afirmação como **verdadeira (V)** ou **falsa (F)**.

| | Afirmação | V/F |
|---|---|---|
| I | "Velhice bem-sucedida", "velhice ativa", "velhice saudável" e "velhice produtiva" nomeiam o mesmo desfecho positivo e carregam forte apelo ideológico. | |
| II | Quando o bom envelhecimento é tratado como mérito pessoal, a conta da desigualdade social é transferida para o próprio idoso. | |
| III | Como agência pessoal e autodeterminação são amplamente demonstradas, elas não encontram limites nas circunstâncias macrossociais. | |
| IV | A probabilidade de exposição a eventos críticos do curso de vida varia fortemente com o status socioeconômico — ou seja, ela não é aleatória. | |

<details>
<summary>Gabarito e comentário</summary>

**I — V.** E foram todos criados quando envelhecer com saúde ainda era exceção, o que ajuda a entender o tom celebratório.

**II — V.** É a crítica central: se envelhecer bem é escolha, quem não envelheceu bem falhou. A responsabilização individual apaga determinantes sociais.

**III — F.** É o limite honesto que a própria teoria contemporânea impõe: sim, há agência; não, ela não é ilimitada. Ela esbarra nas circunstâncias macrossociais.

**IV — V.** Pobreza, isolamento e discriminação por idade expõem os idosos a mais situações estressantes. No Brasil, os eventos críticos aparecem também como problemas de moradia, transporte e segurança — vividos como aborrecimentos constantes, mas com grande chance de se tornarem eventos inesperados e incontroláveis.
</details>

---

## Questão 16 · múltipla escolha · muito difícil
**Objetivo** interpretar a discrepância entre perda documentada e bem-estar relatado.

Nos modelos contemporâneos, o mecanismo de seleção, otimização e compensação ocupa a posição de **mediador** entre as perdas e riscos do envelhecimento (com os recursos pessoais e sociais disponíveis) e os desfechos de bem-estar, atividade, funcionalidade e envolvimento social.

Uma consequência **direta** dessa posição no modelo é que:

**a)** a mediação implica que idosos com perdas relevantes que relatam boa qualidade de vida estão, em regra, superestimando o próprio funcionamento.
**b)** a continuidade da funcionalidade e da satisfação pode ocorrer **na presença** de riscos e perdas biológicas e sociais — o que o modelo chama de paradoxo do bem-estar — porque os recursos pessoais e sociais são convertidos em adaptação por um mecanismo intermediário.
**c)** recursos pessoais e sociais produzem bem-estar diretamente, sem intermediação, e por isso medir esses recursos basta para prever o desfecho.
**d)** adaptação bem-sucedida na velhice exige dispor de recursos excepcionais, o que restringe o desfecho positivo a uma elite.
**e)** o bem-estar subjetivo na velhice é determinado pela quantidade de perdas acumuladas, de modo que mais perdas produzem, necessariamente, menor bem-estar.

<details>
<summary>Gabarito e comentário</summary>

**Gabarito: b**

Se o mecanismo é mediador, então o mesmo nível de perda pode gerar desfechos diferentes conforme a alocação de recursos — que é a definição de mediação. É isso que torna o paradoxo do bem-estar previsível, e não anômalo.

- **a)** é o erro clínico que a aula combate desde o caso do Sr. Anselmo: relato positivo sob perda documentada é hipótese de boa compensação antes de ser hipótese de falta de crítica.
- **d)** contraria a tese de que adaptação bem-sucedida é *viver bem com os recursos disponíveis*, e não dispor de recursos excepcionais.
- **e)** e **c)** descrevem modelos **sem** mediador: efeito direto e proporcional. É exatamente o que o modelo nega.
</details>

---

## Questão 17 · estudo de caso · muito difícil
**Objetivo** integrar biografia, contexto macrossocial e teoria numa mesma leitura.

> **Dona Neuza, 73 anos, encaminhada pela agente comunitária de saúde.**
> **História de vida.** Nasceu no interior do Piauí, terceira de nove irmãos. Estudou até a segunda série; trabalhou em casa de família a partir dos 12 anos e, depois de migrar para São Paulo aos 24, foi cozinheira em restaurante por 31 anos, sem registro em carteira nos primeiros 12. Aposentou-se por idade, com um salário mínimo.
> **Contexto atual.** Mora sozinha num quarto alugado no terceiro andar sem elevador, num bairro com assaltos frequentes na parada de ônibus. Deixou de ir ao posto de saúde depois de dois assaltos na rua e passou a depender da vizinha para comprar remédio. A linha de ônibus que a levava à unidade foi extinta há um ano.
> **Queixa.** "Doutora, eu acho que estou ficando esquecida. Antes eu dava conta de tudo."
> **Observações clínicas.** Vigil, colaborativa, discurso organizado. Descreve com precisão a rotina da vizinha, os horários dos remédios e as contas do mês. Refere sono encurtado e sobressalto a barulhos na rua.
> **Testes.** Desempenho dentro do esperado para a idade e a escolaridade dela na bateria breve aplicada; rastreio de humor sem indicativo de quadro depressivo.

Qual é a formulação mais adequada do caso?

**a)** Dependência comportamental aprendida, mantida pelo reforço social oferecido pela vizinha.
**b)** Comprometimento cognitivo leve de provável etiologia degenerativa, dado o relato de piora funcional referido pela própria paciente.
**c)** Quadro de afastamento, esperado nessa faixa etária, com indicação de estímulo à atividade.
**d)** A queixa cognitiva não encontra correspondência nos testes; a redução da autonomia é mais bem explicada por eventos críticos do curso de vida — não normativos, incontroláveis e distribuídos segundo o status socioeconômico — sobre uma trajetória de baixa escolaridade e baixa renda, e a conduta passa por rede de apoio, segurança e acesso, e não por diagnóstico neurocognitivo.
**e)** Envelhecimento bem-sucedido, já que os testes estão dentro do esperado e não há indicativo de humor deprimido.

<details>
<summary>Gabarito e comentário</summary>

**Gabarito: d**

O caso reúne, de propósito, três camadas que a aula pede que se leiam juntas:

1. **A queixa é um dado, mesmo com testes normais** — mas ela não é, por si, evidência de doença. Aqui, o que a paciente descreve como "esquecimento" é perda de autonomia produzida pelo ambiente: sem transporte e com medo de sair, ela deixou de administrar o que administrava.
2. **Os eventos críticos não são aleatórios.** Extinção de linha de ônibus, insegurança na rua e moradia sem elevador são "aborrecimentos constantes" com alta chance de virar eventos incontroláveis — e a exposição a eles varia com renda e território.
3. **A agência esbarra no macrossocial.** Recomendar "mais atividade" a quem não consegue sair de casa em segurança transfere para ela um problema que não é dela.

Por que as demais falham:
- **a)** confunde apoio contextualmente necessário com padrão dependente aprendido; falta a contingência (aqui, a vizinha compensa uma barreira real, e não reforça um repertório disponível).
- **b)** transforma queixa em diagnóstico, sem alteração objetiva e sem prejuízo funcional atribuível à cognição — e comunicar isso como "início de Alzheimer" produziria dano.
- **c)** naturaliza o desengajamento — o erro que o veredito sobre a teoria do afastamento já corrigiu.
- **e)** lê só o escore e ignora a funcionalidade e o contexto.

**Conduta que o caso sugere:** acionar rede (agente comunitária, unidade de saúde, assistência social), documentar a barreira de acesso, orientar a família e reavaliar em intervalo definido. E deixar registrado no laudo o que foi corrigido, o que não foi e como isso limita a interpretação.
</details>

---

## Questão 18 · ENADE/ENEM · muito difícil
**Objetivo** compreender por que a teoria antecede a coleta de dados.

> "Conhecer teorias antes de coletar dados não é uma opção. É imperativo."
> "O objetivo da psicologia do envelhecimento é estudar os padrões de mudança comportamental associados ao avanço da idade, **distinguindo** aqueles que são típicos da velhice daqueles que são compartilhados por outras idades."
> Anita Liberalesso Neri (2013)

Considerando os dois trechos em conjunto, a afirmação que melhor sustenta a posição da autora é:

**a)** a de que o campo deve abandonar as microteorias e retornar às grandes narrativas da primeira metade do século XX.
**b)** a de que a teoria substitui o dado empírico, já que a interpretação teórica é suficiente para descrever o envelhecimento.
**c)** a de que a distinção proposta é retórica, uma vez que toda mudança observada em idosos decorre do processo de envelhecimento.
**d)** a de que, como a idade é causa do declínio, cabe à teoria apenas descrever a magnitude da perda.
**e)** a de que a teoria decide quais perguntas são pensáveis e o que conta como resultado — de modo que, sem ela, não se sabe o que medir nem o que a medida significa, e a tarefa de *distinguir* o típico da velhice do compartilhado com outras idades fica impossível.

<details>
<summary>Gabarito e comentário</summary>

**Gabarito: e**

Os dois trechos, lidos juntos, formam um argumento: a tarefa do campo é uma tarefa de **discriminação**, e discriminar exige um critério anterior ao dado. O paradigma de ciclo de vida ilustra o custo de não perceber isso — quem parte de "o ápice é a reprodução" já respondeu à pergunta sobre a velhice antes de coletar qualquer dado.

- **a)** contraria a trajetória descrita no capítulo: as grandes teorias cederam espaço a microteorias sobre aspectos particulares — menos grandiosas e muito mais testáveis.
- **b)** exagera até a caricatura; a autora defende teoria *antes* do dado, não *em vez* do dado.
- **d)** e **c)** cometem o erro que a frase da autora existe para impedir: tratar idade como causa e correlação com idade como efeito da idade.

**Fecho da aula:** a velhice deixou de ser o fim da curva; virou uma fase em que ainda se seleciona, otimiza e compensa.
</details>

---

### Chave rápida

| Questão | 1 | 2 | 3 | 4 | 5 | 6 | 7 | 8 | 9 |
|---|---|---|---|---|---|---|---|---|---|
| **Gabarito** | a | V, F, V, V | d | c | e | c | V, V, F, V | a | d |

| Questão | 10 | 11 | 12 | 13 | 14 | 15 | 16 | 17 | 18 |
|---|---|---|---|---|---|---|---|---|---|
| **Gabarito** | b | e | a | c | b | V, V, F, V | b | d | e |
